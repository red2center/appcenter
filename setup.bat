...
powershell -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%START_MENU_SHORTCUT%');$s.TargetPath='%EXE_PATH%';$s.WorkingDirectory='%TARGET_DIR%';$s.Save()"

echo Kisayol olusturuluyor - Masaustunde...
powershell -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%DESKTOP_SHORTCUT%');$s.TargetPath='%EXE_PATH%';$s.WorkingDirectory='%TARGET_DIR%';$s.Save()"
